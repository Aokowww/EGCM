...
context and provides local-power curves for observed Case B and complete-history C1. In
exploratory ADNI
...
0.942, and 1.000 (Figure~\ref{fig:simulation-boundaries}b). The visit-i.i.d. power curve is
not interpreted because its null calibration was invalid. This comparison does not replace
or repair the separately frozen Stage-2b random-slope failure below.
...
represent downstream disease stage rather than baseline confounding. The estimand therefore
asks whether MRI volumes and ADAS13 retain a population-level conditional association after
these demographic and acquisition variables, not whether atrophy causally changes cognition.
...
subject-conditional calculation could be evaluated, the exploratory cohort required at least
...
its 90th percentile was 19 days. This visit-enriched cohort is selected and is not intended
to represent the full ADNI population.
...
were specified before the first GCM analysis. Although the richer visit structure allowed
Design B to be evaluated, we do not report its real-data $p$-values: under bounded visit
counts, its null rejection rates were unacceptably high in the simulation study. The ADNI
analysis therefore uses Design A only.
...
\begin{table}[!ht]
\centering
\caption{Exploratory MRI--ADAS13 Design A results in 647 ADNI participants. Normalised
...
unfavourable random-slope simulation result limit generalisability and make the real-data
conclusions exploratory.

\subsection{What the existing analyses distinguish}

The MRI--ADAS13 analysis does more than assign a sign to each association, but it does not
yet establish predictive biomarker performance. After orienting the three anatomical markers
...
objective. A non-rejection is an empirical answer to the named question, not evidence that
conditional independence has been proved.}
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.20\textwidth}
                              ...
                              >{\raggedright\arraybackslash}X}
\toprule
Completed analysis & Relation to the core objective & Result and defensible conclusion \\
\midrule
...
MRI--ADAS13 & Regional MRI ($X$) and concurrent ADAS13 ($Y$) after demographic and technical adjustment; a supporting application rather than the pathology-beyond-cognition question. & Global empirical $p=0.0001$; all markers survived Holm correction. Supports coherent contemporaneous MRI--cognition signal and method feasibility, but not pathology-specific or prospective biomarker value. \\
...
The completed MRI--AV45 experiment therefore satisfies the core objective for one candidate
modality, one pathology target, and one measured context set. Its primary answer is negative:
the analysis did not detect additional MRI information about amyloid after cognition was known.
This is scientifically useful because it separates a plausible marginal MRI--amyloid signal from
incremental information. It does not establish that MRI is redundant in every population or for
every pathology target: the cohort was complete-case and sparse, non-rejection does not prove
conditional independence, and contemporaneous cognition may represent shared disease information,
mediation, overadjustment, or collider distortion.

The two applications support three levels of interpretation. The global test asks
...
the other markers and demonstrating out-of-subject predictive improvement. Consequently, the
completed MRI analyses only partially distinguish the $X$ variables: they localise signals and provide
descriptive common-scale magnitudes, but they do not yet supply unique-marker tests, formal
pairwise differences, or held-out prediction gains (Table~\ref{tab:adni-core-objective}).
...
combination that best instantiates the paper's core objective. These are exploratory
observational discrimination results, not estimates of causal effects or proof of clinical
utility.
...
25.4\%--29.3\%. These repeated folds assess algorithmic stability within one selected cohort;
...
\begin{table}[!ht]
\centering
\caption{Exploratory unique-marker plasma--amyloid results after conditioning on cognition,
...
The present result is narrower: it concerns incremental information in the specified ADNI
cohort and conditioning set, not clinical qualification of the assay.
...
many-subject C1 branch, which reduces to A1 after adding fixed observed history. It is
not an application of the single-series temporal theory discussed by Shi.
...
\end{enumerate}
P-values will not be used to rank $X$ variables. On a common comparison cohort, oriented
markers will be compared through cross-fitted scale-normalised residual covariances,
...
measured pathology enters $Z$ \cite{janelidze2024,moon2025}. These are frozen qualitative
checks, not criteria for discarding discordant results. An ADNI4 remote extension can treat
...
The completed prospective analysis supplies temporal ordering for a cognitive target, but it
does not replace the pathology-information question linking the IWSM and future wearable
applications. Tau PET and a frozen non-APOE polygenic score remain the next conditional
pathology-information targets.
...
future-cognition analysis. These exploratory measures do not establish causality or clinical
biomarker validity.}
