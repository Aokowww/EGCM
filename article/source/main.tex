...
\author{Qingzi Huo, Marco Simnacher, Xiangnan Xu, and Sonja Greven\\
for the Alzheimer's Disease Neuroimaging Initiative\thanks{A complete listing of ADNI
investigators is provided in the Acknowledgements.}}
...
variables instantiate A1 and its observed-context Case-B and bounded-history C1 reductions
with many participants and sparse visits. The analyses show how
...
distinction in all 20 assignments and the prospective p-tau217 direction in all 20. The
prospective p-tau217 test remained multiplicity-adjusted significant in 19 of 20 assignments.
Cases D and E, and
...
The article makes three contributions. First, it provides a problem map for non-i.i.d. CIT:
five nulls (Cases A--E), three asymptotic regimes inside Case A, and an explicit distinction
between observed, estimated, and latent subject information in Case B. The map identifies the
domain knowledge needed to state the null and separates sampling problems from unresolved
identification problems. Second, it develops a residual-score construction and high-level
rate conditions for Case A1. Observed Case-B information and complete bounded Case-C1 history
reduce to A1 after enlarging the conditioning set; estimated or latent subject information,
growing-history C2, and double asymptotics require additional assumptions. GAMMs, random
forests, kernel methods, neural networks, and pre-trained embeddings can serve as nuisance
learners when the stated prediction and leakage conditions hold. Third, simulations expose
the calibration boundary of the construction and quantify the effects of subject-context
accuracy and history completeness. The ADNI analyses ask whether imaging and plasma
measurements add information beyond named clinical, historical, genetic, and acquisition
variables, while prospective and repeated-fold analyses assess candidate discrimination and
split sensitivity.
...
Cluster-aware cross-fitting and variance estimation have been developed for orthogonal
estimands under multiway clustering \cite{chiang2022}; that work supplies an important
sampling precedent but does not test conditional independence. Nonparametric time-series CIT
has also been constructed from conditional-distribution moment restrictions and a multiplier
bootstrap \cite{songwei2021}.
...
Multiway cluster DML \cite{chiang2022} & orthogonal score for a structural parameter & multiway clustered observations & cluster-aware cross-fitting and cluster-robust variance & sampling precedent, but not a CIT \\
Time-series conditional moments \cite{songwei2021} & conditional-distribution moment restrictions & one dependent series & temporal regularity and multiplier bootstrap & specialised A2/C2 alternative \\
...
\]
where the Case-A1 theorem assumes that these conditional-mean functions are shared across
subjects and visits. Clusters may still differ in visit count, conditional variance, and
within-subject dependence. Subject- or visit-specific conditional means require indexed
nuisance targets and uniform versions of the prediction-rate conditions below. Define the
marginal residuals
\[
...
\end{equation}
For independent, potentially non-identical clusters, the corresponding average population
parameter is
\begin{equation}
\rho_N=\frac1N\sum_{i=1}^N\E(S_i)
=\frac1N\sum_{i=1}^N\E\left[\sum_{t=1}^{m_i}w_{it}
\{X_{it}-f_0(Z_{it})\}\{Y_{it}-g_0(Z_{it})\}\right].
\label{eq:rho}
\end{equation}
Under \eqref{eq:pop-null}, each same-visit conditional covariance is zero and hence
$\rho_N=0$, provided the observation and weighting process satisfies
Assumption~\ref{ass:weights}. For identically distributed clusters, write the common value as
$\rho=\E(S_i)$.

\begin{remark}[What is and is not tested]
Conditional independence implies $\rho_N=0$, but the converse does not hold in general.
...
non-identically distributed in a triangular array through their visit counts, residual
distributions, and within-cluster dependence. The conditional-mean functions $f_0$ and $g_0$
are shared across the array, and the following moment and variance conditions hold uniformly.
\end{assumption}

\begin{assumption}[Observation process, weights, and cluster size]
\label{ass:weights}
The observed visit set and the weights are fixed by design or are measurable with respect to
pre-outcome information included in the scientific context. The weights are nonnegative and
satisfy $\sum_t w_{it}=1$. Under $H_0^{\mathrm{pop}}$, the resulting weighted oracle score is
centred, $\E(S_i)=0$. This condition holds automatically for a fixed visit schedule and fixed
weights under the visit-level null. An outcome-dependent visit process instead requires a
separate argument that centring is preserved after selection. Either
...
The number of folds $K$ is fixed and $n_k/N\to\pi_k\in(0,1)$ for every fold.
\end{assumption}

\begin{assumption}[Product-rate condition]
\label{ass:product-rate}
...
together with the fold assignment. Conditional on $\calT_k$, the held-out cluster terms are
independent and satisfy
\[
\E(C_{g,i}\mid\calT_k)=0,
\qquad
\E(C_{f,i}\mid\calT_k)=0,
\qquad i\in I_k.
\]
Assume further that
\begin{align}
...
\subsection{Null theorem and proof}

\begin{theorem}[Asymptotic null validity of cluster GCM]
\label{thm:null}
Under Assumptions~\ref{ass:clusters}--\ref{ass:variance} and
$H_0^{\mathrm{pop}}$, the statistic in
\eqref{eq:test-statistic} satisfies
\[
T_N\xrightarrow{d}N(0,1).
\]
Consequently, the two-sided test has pointwise asymptotic level $\alpha$.
\end{theorem}
...
Assumptions~\ref{ass:weights} and \ref{ass:centering} give $\E S_i=0$ under
$H_0^{\mathrm{pop}}$. The Lindeberg--Feller central limit theorem for independent triangular
arrays, invoked through Assumption~\ref{ass:oracle-clt}, gives
\[
\frac{N^{-1/2}\sum_iS_i}{\sigma_N}\xrightarrow{d}N(0,1).
\]
All within-subject dependence is contained in $S_i$; the independent units in this limit are
...
For each fold $k$, Assumptions~\ref{ass:crossfit} and \ref{ass:cross-term} imply conditional
centering and independence of the held-out cluster terms. Therefore
\begin{align*}
\E\left[\left.
\left\{\frac1{\sqrt{n_k}}\sum_{i\in I_k}C_{g,i}\right\}^2
\right|\calT_k\right]
&=\frac1{n_k}\sum_{i\in I_k}\E(C_{g,i}^2\mid\calT_k)
=B_{g,k}^{\mathrm{cl}}=\op(1),\\
\E\left[\left.
\left\{\frac1{\sqrt{n_k}}\sum_{i\in I_k}C_{f,i}\right\}^2
\right|\calT_k\right]
&=B_{f,k}^{\mathrm{cl}}=\op(1).
\end{align*}
Conditional Markov's inequality yields
\[
\frac1{\sqrt{n_k}}\sum_{i\in I_k}C_{g,i}=\op(1),
\qquad
\frac1{\sqrt{n_k}}\sum_{i\in I_k}C_{f,i}=\op(1)
\]
for every $k$. Since $K$ is fixed and $n_k/N\to\pi_k$, summing these foldwise terms with
weights $\sqrt{n_k/N}$ gives
\[
N^{-1/2}\sum_iC_{g,i}=\op(1),
\qquad
N^{-1/2}\sum_iC_{f,i}=\op(1).
\]
This foldwise argument never conditions jointly on overlapping training samples.

\textbf{Step 4: Studentisation.}
Assumption~\ref{ass:variance} and its law of large numbers give
\[
\widehat\sigma_N^2
=\frac1N\sum_i(S_i-\overline S_N^0)^2+\op(1)
=\sigma_N^2+\op(1)
\xrightarrow{\mathbb P}\sigma^2.
\]
Steps 2 and 3 make every estimated-nuisance remainder in
\eqref{eq:cluster-numerator-decomp} $\op(1)$. Combining Step 1 with the preceding variance
limit and applying Slutsky's theorem gives $T_N\Rightarrow N(0,1)$. The stated level result
follows from continuity of the standard normal distribution.
\end{proof}

\subsection{Fixed-marker global test}

The application tests a finite, pre-specified marker set jointly. Write
$\bm S_i=(S_{i1},\ldots,S_{ip})^\top$ for the oracle subject-score vector and
$\widehat{\bm S}_i$ for its cross-fitted version. Let
\[
\Sigma_N=\frac1N\sum_{i=1}^N\Cov(\bm S_i),
\qquad
\widehat\Sigma_N=\frac1N\sum_{i=1}^N
(\widehat{\bm S}_i-\overline{\widehat{\bm S}})
(\widehat{\bm S}_i-\overline{\widehat{\bm S}})^\top,
\]
and let $D_N$ and $\widehat D_N$ be their diagonal matrices.

\begin{proposition}[Gaussian multiplier validity for a fixed marker set]
\label{prop:multiplier}
Suppose $p$ is fixed, the joint null holds for all $p$ marker scores, and the componentwise
versions of Assumptions~\ref{ass:clusters}--\ref{ass:variance} hold. Assume additionally a
multivariate Lindeberg condition,
$\Sigma_N\to\Sigma$ for a positive-definite matrix $\Sigma$, and
$\widehat\Sigma_N-\Sigma_N=\op(1)$ entrywise. Then
\[
\bm T_N=\sqrt N\,\widehat D_N^{-1/2}
\overline{\widehat{\bm S}}
\xrightarrow{d}N_p(\bm 0,R),
\qquad
R=D^{-1/2}\Sigma D^{-1/2},
\]
where $D=\operatorname{diag}(\Sigma)$. Conditional on the data, the shared-subject Gaussian
multiplier vector
\[
\bm T_N^*=\frac1{\sqrt N}\widehat D_N^{-1/2}
\sum_{i=1}^Ne_i(\widehat{\bm S}_i-\overline{\widehat{\bm S}}),
\qquad e_i\stackrel{\mathrm{i.i.d.}}{\sim}N(0,1),
\]
consistently estimates the null law of $\max_{j\leq p}|T_{Nj}|$. Consequently, the
multiplier global test is asymptotically level $\alpha$ when its Monte Carlo size tends to
infinity after $N$.
\end{proposition}

\begin{proof}
For every fixed $a\in\R^p$, the scalar proof of Theorem~\ref{thm:null} applied to
$a^\top\bm S_i$ makes the cross-fitted remainder $\op(1)$ and gives a Lindeberg--Feller
limit. The Cram\'er--Wold device therefore yields
$\sqrt N\,\overline{\widehat{\bm S}}\Rightarrow N_p(\bm0,\Sigma)$.
Entrywise covariance consistency and positive definiteness imply
$\widehat D_N\to_{\mathbb P}D$, so Slutsky's theorem gives the stated limit for $\bm T_N$.
Conditional on the observed scores, $\bm T_N^*$ is exactly Gaussian with covariance
$\widehat R_N=\widehat D_N^{-1/2}\widehat\Sigma_N\widehat D_N^{-1/2}$.
Because $\widehat R_N\to_{\mathbb P}R$ and the maximum of a non-degenerate finite Gaussian
vector has a continuous distribution, the conditional distribution of
$\max_j|T_{Nj}^*|$ converges to the same limit as $\max_j|T_{Nj}|$. Monte Carlo convergence
then gives the final statement.
\end{proof}

This proposition covers the fixed marker panels used below. A growing marker dictionary
requires high-dimensional moment, rate, covariance-estimation, and anti-concentration
conditions beyond the fixed-$p$ argument.
...
\begin{theorem}[Asymptotic linearity under alternatives]
\label{thm:alternative}
Let $\mu_{i,N}=\E(S_i)$ and $\rho_N=N^{-1}\sum_i\mu_{i,N}$. Assume the analogues of
Assumptions~\ref{ass:oracle-clt}--\ref{ass:variance} for the centred oracle scores
$S_i-\mu_{i,N}$, and assume
\[
\frac1N\sum_{i=1}^N(\mu_{i,N}-\rho_N)^2=o(1).
\]
Then
\[
...
\xrightarrow{d}N(0,1).
\]
Consequently, alternatives with $\sqrt N|\rho_N|\to\infty$ are detected consistently. If
$\sqrt N\rho_N\to c$ and $\widehat\sigma_N\to_{\mathbb P}\sigma_A\in(0,\infty)$, then
$T_N\Rightarrow N(c/\sigma_A,1)$.
\end{theorem}

\begin{proof}
Apply the four steps in the proof of Theorem~\ref{thm:null} to
$S_i-\mu_{i,N}$. The assumed mean-heterogeneity condition makes the between-cluster mean
component of the empirical score variance negligible, so the studentising variance has the
same limit as the centred oracle variance. Thus
$\sqrt N(\overline S_N-\rho_N)/\widehat\sigma_N\Rightarrow N(0,1)$.
For $T_N$, add $\sqrt N\rho_N/\widehat\sigma_N$ and apply Slutsky's theorem. This term
diverges in absolute value under the first stated alternative and converges to
$c/\sigma_A$ under the local alternative.
\end{proof}
...
products as independent visits inflates rejection. The dashed line marks the nominal 0.05
level. The dotted 0.075 line is the frozen maximum acceptable upper confidence limit: a null
setting passes only when the upper end of its exact 95\% interval lies at or below that line.
(b) Local power for the A1 subject-score, observed
...
correction. Holm adjustment was applied within each pre-specified marker family. The separate
ADNI targets and alignment windows are exploratory analyses, and no across-application
family-wise error claim is made.
...
prediction gains. In the common cohort, plasma added substantially more predictive information
than structural MRI; p-tau217 also improved future-cognition prediction under both alignment
windows. A repeated-fold diamond and whisker are shown for the primary future-cognition
analysis.}
...
Three limitations remain central. The A1 theorem is expressed through high-level nuisance-rate
and observation-process conditions; learner-specific corollaries must verify them for each
implementation. The scalar residual covariance is not omnibus against conditional-dependence
alternatives with zero conditional covariance. Finally, the ADNI findings are observational,
complete-case, and exploratory; they establish neither causality nor clinical biomarker
qualification. These limits determine the next theoretical and empirical steps rather than
changing the five-case taxonomy.
...
2026 UPENN plasma and UC Berkeley amyloid-PET Study Files snapshots. Plasma analytes were
treated as continuous research-use-only measures; no FDA-cleared Lumipulse ratio threshold
or clinical diagnostic cut-off was applied. Restricted rows were
...
informed consent was obtained under those protocols \cite{weiner2010}. Journal submission
will follow ADNI Data and Publications Committee review requirements.

% ============================================================
\section{Limitations and next directions}

The A1 theorem uses high-level shared-mean, observation-process, prediction-rate, and
variance conditions. Learner-specific corollaries still need to verify these conditions for
particular GAMM, kernel, or neural nuisance estimators. In particular, an outcome-dependent
visit process is covered only when weighted score centring can be justified after selection.
Any response to the original Stage-2b random-slope failure must be evaluated under a newly
frozen method version rather than tuned against that row.
...
\midrule
A1 & state-level, many subjects & high-level theorem and proof under cluster rates & Stage 2a; six stress settings; ADNI & primary current scope \\
...
\bibitem{chiang2022}
H. D. Chiang, K. Kato, Y. Ma, and Y. Sasaki.
\newblock Multiway cluster robust double/debiased machine learning.
\newblock \emph{Journal of Business \& Economic Statistics}, 40(3):1046--1056, 2022.
\newblock doi:10.1080/07350015.2021.1895815.

\bibitem{songwei2021}
X. Song and H. Wei.
\newblock Nonparametric tests of conditional independence for time series.
\newblock arXiv:2110.04847, 2021.
\newblock \url{https://arxiv.org/abs/2110.04847}.
...
\newblock \url{https://www.ucl.ac.uk/mathematical-physical-sciences/statistical-science-seminars-2024-25}.
...
\newblock \emph{JAMA Neurology}, 81(9):947--957, 2024.
\newblock doi:10.1001/jamaneurol.2024.2619.
...
\newblock \emph{Alzheimer's \& Dementia}, 20(10):7248--7262, 2024.
\newblock doi:10.1002/alz.14206.
